...
open class TerminalView: NSView, NSTextInputClient, NSUserInterfaceValidations, TerminalDelegate {
    ...
    public var contentInsetLeft: CGFloat = 0 {
        didSet {
            guard oldValue != contentInsetLeft else { return }
    ...
    private var displayUpdateBatchDepth = 0
    private var batchedScrollNotificationPending = false

    func startDisplayUpdates ()
    {
        displayUpdateBatchDepth += 1
    }
    
    func suspendDisplayUpdates()
    {
        if displayUpdateBatchDepth > 0 {
            displayUpdateBatchDepth -= 1
        }
        guard displayUpdateBatchDepth == 0, batchedScrollNotificationPending else {
            return
        }
        batchedScrollNotificationPending = false
        emitScrolledNotification()
    }
    ...
    {
        return NSRect (x: 0, y: 0, width: contentInsetLeft + cellDimension.width * CGFloat(terminal.cols) + reservedScrollerWidth, height: cellDimension.height * CGFloat(terminal.rows))
    }

    func getEffectiveWidth (size: CGSize) -> CGFloat
    {
        max(0, size.width - reservedScrollerWidth - contentInsetLeft)
    }
    
    private func emitScrolledNotification() {
        updateScroller()
    ...
    open func scrolled(source terminal: Terminal, yDisp: Int) {
        ...
        guard displayUpdateBatchDepth == 0 else {
            batchedScrollNotificationPending = true
            return
        }
        emitScrolledNotification()
    }
    ...
    open func linefeed(source: Terminal) {
        ...
        if allowMouseReporting && selection.active {
    ...
    func calculateMouseHit (at point: CGPoint) -> (grid: Position, pixels: Position)
    {
        ...
        let displayBuffer = terminal.displayBuffer
        let col = Int ((point.x - contentInsetLeft) / cellDimension.width)
