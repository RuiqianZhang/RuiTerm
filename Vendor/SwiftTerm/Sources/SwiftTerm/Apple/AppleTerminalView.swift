...
extension TerminalView {
    ...
    func setupOptions(width: CGFloat, height: CGFloat)
    {
        ...
        let terminalOptions = zeroSizedView
            ...
            : TerminalOptions(cols: Int(getEffectiveWidth(size: CGSize(width: width, height: height)) / cellDimension.width),
    ...
    func drawTerminalContents (dirtyRect: TTRect, context: CGContext, bufferOffset: Int)
    {
        ...
        for row in firstRow...lastRow {
            ...
            let lineOrigin = CGPoint(x: contentInsetLeft, y: frame.height - lineOffset)
    ...
    func updateCursorPosition()
    {
        ...
        let lineOrigin = CGPoint(x: contentInsetLeft, y: offset)
        #else
        let offset = (cellDimension.height * (CGFloat(buffer.y-(buffer.yDisp-buffer.yBase)+1)))
        let lineOrigin = CGPoint(x: contentInsetLeft, y: frame.height - offset)
        #endif
    ...
    func queuePendingDisplay ()
    {
        ...
        if !pendingDisplay {
            ...
            let fpsDelay = fps30
            pendingDisplay = true
            SyncDebug.log("queue-scheduled (+33.34ms)")
