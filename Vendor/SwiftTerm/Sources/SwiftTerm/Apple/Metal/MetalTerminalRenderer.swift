...
final class MetalTerminalRenderer: NSObject, MTKViewDelegate {
    ...
    private var fontPostScriptNameCache: [ObjectIdentifier: String] = [:]
    ...
    func draw(in view: MTKView) {
        ...
        if MetalTerminalRenderer.profileEnabled && elapsed >= 1.0 {
    ...
    private func buildRowDrawData(row: Int,
        ...
        let lineOrigin = CGPoint(x: terminalView.contentInsetLeft, y: terminalView.bounds.height - lineOffset)
    ...
    private func glyphEntry(for font: CTFont, glyph: CGGlyph) -> GlyphEntry? {
        let key = GlyphKey(fontName: postScriptName(for: font),
    ...
    private func postScriptName(for font: CTFont) -> String {
        let identifier = ObjectIdentifier(font as AnyObject)
        if let cached = fontPostScriptNameCache[identifier] {
            return cached
        }
        let name = CTFontCopyPostScriptName(font) as String
        if fontPostScriptNameCache.count > 512 {
            fontPostScriptNameCache.removeAll(keepingCapacity: true)
        }
        fontPostScriptNameCache[identifier] = name
        return name
    }
