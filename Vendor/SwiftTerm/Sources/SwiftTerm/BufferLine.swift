...
public final class BufferLine: CustomDebugStringConvertible {
    ...
    @inline(__always)
    func setAsciiRun(start: Int, bytes: ArraySlice<UInt8>, attribute: Attribute) {
        guard start < dataSize, !bytes.isEmpty else { return }
        let count = min(bytes.count, dataSize - start)
        guard count > 0 else { return }
        var byteIndex = bytes.startIndex
        for offset in 0..<count {
            data[start + offset] = CharData(attribute: attribute, code: Int32(bytes[byteIndex]), size: 1)
            byteIndex += 1
        }
        bump()
    }
