...
open class TerminalView: UIScrollView, UITextInputTraits, UIKeyInput, UIScrollViewDelegate, TerminalDelegate, UIPointerInteractionDelegate {
    ...
    {
        return CGRect (x: 0, y: 0, width: contentInsetLeft + cellDimension.width * CGFloat(terminal.cols), height: cellDimension.height * CGFloat(terminal.rows))
    }
    ...
    {
        return max(0, size.width - contentInsetLeft)
    }
